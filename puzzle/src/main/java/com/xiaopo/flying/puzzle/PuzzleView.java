...
public class PuzzleView extends FrameLayout {
    ...
    private ArrayList<Pair<CardView, LayoutParams>> listCards = new ArrayList<>();
...
//        canvas.drawColor(Color.BLACK);
    ...
    private void addViewToList(CardView card) {
        listCards.add(new Pair<CardView, LayoutParams>(card, null));
    }
    ...
    private void refreshViews() {
        if (!listCards.isEmpty()) {
            ...
            for (int i = 0; i < listCards.size(); i++) {
                Pair<CardView, LayoutParams> pairCardView = listCards.get(i);
                if (pairCardView.second == null) {
                    int wi = pairCardView.first.getDrawable().getIntrinsicWidth();
                    int he = pairCardView.first
                    ...
                    listCards.set(i, new Pair<>(pairCardView.first, params));
                }
                if (isCollageFixed)
                    listCards.get(i).first.setFixedItem();

                this.addView(listCards.get(i).first, listCards.get(i).second);
