...
public class PuzzlePreview extends View {

    private static final String TAG = "PuzzleView";

    private final Random random = new Random();

    private PuzzleView.ActionMode currentMode = PuzzleView.ActionMode.NONE;
    ...
    private PuzzleView.OnPieceSelectedListener onPieceSelectedListener;

    private Runnable switchToSwapAction = new Runnable() {
        @Override
        public void run() {
            if (!canSwap) return;
            currentMode = PuzzleView.ActionMode.SWAP;
            invalidate();
        }
    };
    ...
    public PuzzlePreview(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public PuzzlePreview(Context context, AttributeSet attrs, int defStyleAttr) {
...
//        canvas.drawColor(Color.BLACK);
        ...
        for (int i = 0; i < puzzleLayout.getAreaCount(); i++) {
            ...
            if (piece == handlingPiece && currentMode == PuzzleView.ActionMode.SWAP) {
                continue;
            }
        ...
        // draw selected area
        if (handlingPiece != null && currentMode != PuzzleView.ActionMode.SWAP) {
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (event.getAction() & MotionEvent.ACTION_MASK) {
            ...
            case MotionEvent.ACTION_UP:
                finishAction(event);
                currentMode = PuzzleView.ActionMode.NONE;
                removeCallbacks(switchToSwapAction);
                break;
        }
    ...
    private void decideActionMode(MotionEvent event) {
        ...
        if (event.getPointerCount() == 1) {
            ...
            if (handlingLine != null && canMoveLine) {
                currentMode = PuzzleView.ActionMode.MOVE;
            } else {
                handlingPiece = findHandlingPiece();

                if (handlingPiece != null && canDrag) {
                    currentMode = PuzzleView.ActionMode.DRAG;
            ...
            if (handlingPiece != null
                ...
                && currentMode == PuzzleView.ActionMode.DRAG
                && canZoom) {
                currentMode = PuzzleView.ActionMode.ZOOM;
            }
        }
    }
    ...
    public void addPiece(Drawable drawable, Matrix initialMatrix, String path) {
        ...
        if (drawable == null) {
            initialMatrix = MatrixUtils.generateCenterCropMatrix(area, (int) puzzleLayout.width(),
                (int) puzzleLayout.height(), 0f);
        }
    ...
    public void setOnPieceSelectedListener(PuzzleView.OnPieceSelectedListener onPieceSelectedListener) {
