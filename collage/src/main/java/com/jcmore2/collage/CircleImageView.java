...
public class CircleImageView extends ImageView {
    ...
    public CircleImageView(Context context, AttributeSet attrs, int defStyle) {
        ...
        fixRadius = a.getDimensionPixelSize(R.styleable.CircleImageView_civ_radius, -1);
        mBorderColor = a
    ...
    public void setFixRadius(int fixRadius) {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (mFillColor != Color.TRANSPARENT) {
            if (fixRadius > 0) {
                canvas.drawRoundRect(mDrawableRect, fixRadius, fixRadius, mFillPaint);
            } else {
        ...
        if (fixRadius > 0) {
            float perX = mBorderRect.width() * (fixRadius / 100f);
            canvas.drawRoundRect(mDrawableRect, perX, perX, mBitmapPaint);
        } else {
            canvas.drawCircle(mDrawableRect.centerX(), mDrawableRect.centerY(),
                mDrawableRadius,
                mBitmapPaint);
        ...
        if (mBorderWidth > 0) {
            if (fixRadius > 0) {
                float perX = mBorderRect.width() * (fixRadius / 100f);
                mBorderPaint.setStrokeWidth(perX);
                canvas.drawRoundRect(mBorderRect, perX, perX, mBorderPaint);
            } else {
                canvas.drawCircle(mBorderRect.centerX(), mBorderRect.centerY(), mBorderRadius,
                    mBorderPaint);
            }
        }
        Rect r = getDrawable() != null ? getDrawable().getBounds() : new Rect();

        if (!TextUtils.isEmpty(text)) {
            // draw text
            int width = getWidth() < 0 ? r.width() : getWidth();
            int height = getHeight() < 0 ? r.height() : getHeight();
            float fontSize = (Math.min(width, height) / 2.5f);
